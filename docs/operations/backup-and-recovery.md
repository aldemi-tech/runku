...
> **Application file boundary:** the packaged compact helper includes the dedicated
...
When `RUNKU_PLATFORM_DATABASE_URL` is configured, PostgreSQL replaces only the first row's local
