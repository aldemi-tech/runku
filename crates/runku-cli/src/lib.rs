...
/// Authenticated local-to-Management scope binding help appended by the executable.
pub const LINK_HELP: &str = "\nREMOTE PROJECT LINK:\n  runku link [--root PATH] [--workspace REF] [--listen LOOPBACK:PORT] [--project-id prj_* --environment-id env_*]\n  Without IDs, link lists the environments authorized by the current login and prompts on a terminal.\n  Explicit IDs remain available for scripts and non-interactive use.\n";
...
pub enum CliCommand {
    ...
    /// Verify remote authority before binding one local root to a Product scope.
    Link {
        /// Existing project directory.
        root: PathBuf,
        /// Durable default Workspace.
        ...
        /// Explicit loopback listener persisted in local state.
        listen: SocketAddr,
        /// Exact remotely authorized Product scope; absent selects from the authenticated catalog.
        scope: Option<EnvironmentScope>,
    },
...
fn parse_link(args: Vec<OsString>) -> Result<CliCommand, CliUsageError> {
    let mut flags = Flags::new(args)?;
    let root = flags.project_root()?;
    let workspace = flags
        .optional_string("--workspace")?
...
mod tests {
    ...
    fn project_commands_default_root_to_current_directory() {
        ...
        assert!(matches!(
            ...
            Ok(CliCommand::Link { root, .. }) if root == current
        ));
        assert!(matches!(
    ...
    fn parses_every_command_and_repeated_origins() {
        ...
        assert!(matches!(
            parse_args(args(&["link", "--root", "/tmp/project"])),
            Ok(CliCommand::Link { scope: None, .. })
        ));
        assert!(
            parse_args(args(&[
                "link",
    ...
    fn rejects_unknown_duplicate_missing_positional_and_non_loopback_values() {
        for invalid in [
            ...
            args(&["link", "--project-id", "prj_00000000000000000000000001"]),
            args(&[
                "link",
                "--project-id",
                "prj_00000000000000000000000001",
