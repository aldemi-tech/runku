...
check: toolchain-check fmt-check lint test docs incomplete-check sdk-typescript-check sdk-server-check sdk-react-check cli-package-check release-package-check selfhost-package-check chat-example-check field-board-example-check node-example-check
...
# concurrency, database, Docker, and example execution remain explicit maintainer gates.
ci-check: ci-rust-check ci-packages-check

ci-rust-check: toolchain-check fmt-check incomplete-check
...
docs: js-install
	...
	pnpm --filter @runku/docs typecheck
	pnpm --filter @runku/docs build
...
install-cli-check:
	...
	"$$install_root/bin/runku" --version | rg -x 'runku 0\.5\.4'; \
...
field-board-example-check: js-install sdk-typescript-check sdk-server-check sdk-react-check
	cargo build -p runku-cli --release --locked
	$(CURDIR)/target/release/runku init --root examples/field-board-next
	$(CURDIR)/target/release/runku dev --root examples/field-board-next --prepare --public-env-prefix NEXT_PUBLIC_RUNKU_
	$(CURDIR)/target/release/runku build --root examples/field-board-next
	cd examples/field-board-next && RUNKU_BIN="$(CURDIR)/target/release/runku" pnpm check
...
# Explicit operator-provided YugabyteDB YSQL conformance gate. Use a disposable database in the
# intended version and topology; this target does not change cluster membership.
product-yugabyte-check:
	@test -n "$(RUNKU_TEST_YUGABYTE_URL)" || { echo "RUNKU_TEST_YUGABYTE_URL is required"; exit 1; }
	RUNKU_TEST_YUGABYTE_URL="$(RUNKU_TEST_YUGABYTE_URL)" cargo test -p runku-data-postgres --test postgres_conformance yugabytedb_ysql_common_logical_store_conformance --locked -- --exact --test-threads=1

# Explicit external-client gate; kept outside ci-check because it requires the separately installed
# official AWS CLI and starts a loopback Product listener.
object-storage-s3-client-check:
	@command -v aws >/dev/null 2>&1 || { echo "the official AWS CLI is required"; exit 1; }
	RUNKU_TEST_AWS_CLI=1 cargo test -p runku-local s3::tests::official_aws_cli_conformance_when_enabled --locked -- --exact --nocapture
...
sdk-react-check: js-install sdk-typescript-check
	cd packages/react && pnpm check
...
release-schema-coexistence-check:
	cargo test -p runku-local lifecycle::tests::optional_schema_views_share_channels_and_rollback_while_required_addition_blocks --locked -- --exact
	cargo test -p runku-runtime --test runtime_conformance release_schema_view_projects_reads_and_preserves_unknown_fields_on_replace --locked -- --exact
	cargo test -p runku-server product::tests::console_serving_policy_uses_manifest_evidence_cas_and_operation_replay --locked -- --exact
	cargo test -p runku-file-storage tests::logical_object_composition_streams_retries_ranges_and_cancels --locked -- --exact
