...
| Environment lifecycle authority | Domain service, SQLite/PostgreSQL repository, exact-scope create/get/update/archive/restore/operation Management API, and compact serving stop/start composition implemented and test-covered; provider materialization remains a separate Cloud gate |
...
| Logical Object Storage authority | Revisioned bucket/object/access-key domain, SQLite/PostgreSQL metadata authority, content-addressed filesystem/S3 bytes, authenticated console transfer/browser API, and a shared SigV4 Product path for ListObjectsV2/HEAD/GET/PUT/COPY/DELETE, public reads, presigned URLs, CORS, and one-time keys are implemented/test-covered; multipart, lifecycle execution, coordinated backup, native SDK, and CLI surfaces are not yet implemented |
